...
def create_fan(base_center, angle_rot_XY, area_first, length_fan, height_ingate,\
    ...
    for i in range(number_segs_fan):
        ...
        object_id = create_trapezoidal_fan_seg(base_lengths_per_seg_list[i], \
                height_per_seg_list[i], top_lengths_per_seg_list[i], [1,0,0], \
                length_per_seg, length_factor_per_seg, base_centers_per_seg_near_origin_list[i], \
                0., 'Fan_trapezoid_'+str(counter)+"___"+str(i))
        faces_vars_list.append([object_id, 'Fan_trapezoid_'+str(counter)+"___"+str(i)])
    
    
    # CREATE SIMPLE SOLID SEGMENTS FROM THE TRAPEZOIDS, PROBLEM IS THAT WE CANNOT APPLY FILLETS TO EACH SEGMENT. WE'LL GET UNDEFORMABLE FACES THIS WAS
    # we create the solid segments from the Faces, we dont even have to geompy.addToStudy(Face_2, name_object) in create_trapezoidal_fan_seg!
    accuracy = 1E-2
    radius = 0.5
    # solids_vars_list = []
    # for i in range(len(faces_vars_list)-1):
    #     # we need two consecutive trapezoids
    #     objectname0 = faces_vars_list[i][0]
    #     objectname1 = faces_vars_list[i+1][0]
    #     Fan_segment_1___0_vertex_9 = geompy.GetSubShape(objectname0, [9])
    #     Fan_segment_1___1_vertex_9 = geompy.GetSubShape(objectname1, [9])
    #     Line_1 = geompy.MakeLineTwoPnt(Fan_segment_1___0_vertex_9, Fan_segment_1___1_vertex_9)
    #     Fan_segment_1___0_vertex_7 = geompy.GetSubShape(objectname0, [7])
    #     Fan_segment_1___1_vertex_7 = geompy.GetSubShape(objectname1, [7])
    #     Line_2 = geompy.MakeLineTwoPnt(Fan_segment_1___0_vertex_7, Fan_segment_1___1_vertex_7)
    #     Fan_segment_1___0_vertex_4 = geompy.GetSubShape(objectname0, [4])
    #     Fan_segment_1___1_vertex_4 = geompy.GetSubShape(objectname1, [4])
    #     Line_3 = geompy.MakeLineTwoPnt(Fan_segment_1___0_vertex_4, Fan_segment_1___1_vertex_4)
    #     Fan_segment_1___0_vertex_5 = geompy.GetSubShape(objectname0, [5])
    #     Fan_segment_1___1_vertex_5 = geompy.GetSubShape(objectname1, [5])
    #     Line_4 = geompy.MakeLineTwoPnt(Fan_segment_1___0_vertex_5, Fan_segment_1___1_vertex_5)
    #     Fan_segment_1___0_edge_8 = geompy.GetSubShape(objectname0, [8])
    #     Fan_segment_1___1_edge_8 = geompy.GetSubShape(objectname1, [8])
    #     Wire_1 = geompy.MakeWire([Fan_segment_1___0_edge_8, Fan_segment_1___1_edge_8, Line_1, Line_2], 0.001)
    #     Fan_segment_1___0_edge_6 = geompy.GetSubShape(objectname0, [6])
    #     Fan_segment_1___1_edge_6 = geompy.GetSubShape(objectname1, [6])
    #     Wire_1_edge_9 = geompy.GetSubShape(Wire_1, [9])
    #     Wire_2 = geompy.MakeWire([Fan_segment_1___0_edge_6, Fan_segment_1___1_edge_6, Line_4, Wire_1_edge_9], 0.001)
    #     Fan_segment_1___0_edge_3 = geompy.GetSubShape(objectname0, [3])
    #     Fan_segment_1___1_edge_3 = geompy.GetSubShape(objectname1, [3])
    #     Wire_2_edge_9 = geompy.GetSubShape(Wire_2, [9])
    #     Wire_3 = geompy.MakeWire([Fan_segment_1___0_edge_3, Fan_segment_1___1_edge_3, Line_3, Wire_2_edge_9], 0.001)
    #     Fan_segment_1___0_edge_10 = geompy.GetSubShape(objectname0, [10])
    #     Fan_segment_1___1_edge_10 = geompy.GetSubShape(objectname1, [10])
    #     Wire_1_edge_5 = geompy.GetSubShape(Wire_1, [5])
    #     Wire_3_edge_9 = geompy.GetSubShape(Wire_3, [9])
    #     Wire_4 = geompy.MakeWire([Fan_segment_1___0_edge_10, Fan_segment_1___1_edge_10, Wire_1_edge_5, Wire_3_edge_9], 0.001)
    ...
    #     Shell_1 = geompy.MakeShell([objectname0, objectname1, Face_1, Face_2, Face_3, Face_4])
    #     Solid_1 = geompy.MakeSolid([Shell_1])
    #     ##Fillet_1 = geompy.MakeFillet(Solid_1, radius, geompy.ShapeType["EDGE"], [15, 19])
    #     ##Limit_tolerance_1 = geompy.LimitTolerance(Fillet_1, accuracy)
    #     ##geompy.addToStudy( Solid_1, 'Fan_segment_'+str(counter)+"___"+str(i) )
    #     solids_vars_list.append([ Solid_1, 'Fan_segment_'+str(counter)+"___"+str(i) ])
...
def create_trapezoidal_fan_seg(base_length, height, top_length, vector_direction,\
                        channel_length, length_factor, base_center, \
                        rot_deg_around_z, name_object):
    """
    Creates a trapezoidal fan segment for the constant cross-section fan. Currently
    only the generated faces are used (nothing is extruded in the current version).
    ...
    vector_direction : TYPE list of float
        DESCRIPTION. vector of extrusion (will mostly be the x-axis, currently unused).
    channel_length : TYPE float
        DESCRIPTION. length of fan segment (currently unused).
    length_factor : TYPE float
        DESCRIPTION. determines the length progression when extruding is used 
                     (currently unused).
    ...
    rot_deg_around_z : TYPE float
        DESCRIPTION. angle in degrees for rotation the fan segment (currently unused).
    ...
    Face_2 : TYPE Salome object
        DESCRIPTION. the returned Salome is needed to keep track of it. 

    """

    # for the ingate fan we will have to stretch the channel segments, because
    # in the fan, the segments will be very short. we will stretch them by 
    # a stretch_factor. In the end we will have to scale back again by the same
    # stretch_factor. All because Salome cannot deal with very short channels
    # and small area progressions.
    # would also be a feature in the other script :(
    ...
    # geompy.addToStudy(Face_2, name_object)
...
# point_list15 = [[10, 10, 0], [50, 25, 0], [100, 30, 0], [150, -50, 0] ]
# create_continuous_channel(point_list15, 100, 0.9, 1.0, 15)

################### CREATE THE INLET FANS FOR THE RUNNER ######################
create_fan(point_list4[-1], 0, 22.68, 25, 1, 0.9, 1) # for area_first just chose last area of channel!
create_fan(point_list5[-1], 0, 22.68, 20, 1.2, 0.9, 2) # values are printed in Salome Python shell
create_fan(point_list6[-1], 0, 22.68, 20, 1.2, 0.9, 3)
create_fan(point_list7[-1], 0, 22.68, 25, 1, 0.9, 4)
...
# level 3
point_list4 = [ [112.5, 50, 0], [140, 30, 0], [145, 30, 0] ]
point_list5 = [ [112.5, 50, 0], [140, 75, 0], [145, 75, 0] ]
point_list6 = [ [115, 130, 0], [140, 105, 0], [145, 105, 0] ]
point_list7 = [ [115, 130, 0], [140, 150, 0], [145, 150, 0] ]
